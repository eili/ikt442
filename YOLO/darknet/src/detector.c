...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
                   float hier_thresh, int dont_show, int ext_output, int save_labels, char* detectClass)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    ...
    while(1){
		...
		draw_detections_v4(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output, detectClass);
...
void run_detector(int argc, char **argv)
{
	...
	//Special case for the "fallen person" test
	else if (0 == strcmp(argv[2], "fp")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, "person");
